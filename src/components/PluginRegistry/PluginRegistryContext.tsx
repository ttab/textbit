...
export const PluginRegistryContextProvider = ({ children, plugins }: PropsWithChildren & { plugins: Plugin.Definition[] }): JSX.Element => {
  const initPlugins = initializePlugins(initialState, plugins)
...
// Add one plugin
function addPlugin(state: PluginRegistryProviderState, newPlugin: Plugin.Definition): {
  ...
  const plugins = registerPlugin(state.plugins, newPlugin)
  ...
  const leafComponents = (newPlugin?.componentEntry && ceClass === 'leaf')
    ? registerComponents(state.leafComponents, newPlugin.componentEntry?.type || newPlugin.name, newPlugin.componentEntry)
    : state.leafComponents
  const elementComponents = (newPlugin?.componentEntry && ceClass !== 'leaf')
    ? registerComponents(state.elementComponents, newPlugin.componentEntry?.type || newPlugin.name, newPlugin.componentEntry)
...
function initializePlugins(state: PluginRegistryProviderState, plugins: Plugin.Definition[]): {
  plugins: Plugin.Definition[]
  actions: PluginRegistryAction[]
  leafComponents: Map<string, PluginRegistryComponent>
  elementComponents: Map<string, PluginRegistryComponent>
  ...
  plugins.forEach(p => {
    state.plugins = registerPlugin(state.plugins, p)
    state.actions = registerActions(state.plugins)

    const ceClass = p?.componentEntry?.class

    if (p?.componentEntry && ceClass === 'leaf') {
      state.leafComponents = registerComponents(state.leafComponents, p.componentEntry?.type || p.name, p.componentEntry)
    } else if (p?.componentEntry && ceClass !== 'leaf') {
      state.elementComponents = registerComponents(state.elementComponents, p.componentEntry?.type || p.name, p.componentEntry)
    }
  })
